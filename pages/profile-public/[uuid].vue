            ...
            <div>
              ...
              <div class="flex gap-2">
                <a
                  ...
                  target="_blank"
                  class="border w-full border-gray-600 text-gray-600 py-2 px-4 rounded-md text-sm text-center font-medium hover:bg-gray-800 hover:text-yellow-500 transition-colors disabled:cursor-not-allowed">
                  {{ userData.phone_number }}
                </a>
              </div>
            </div>
...
onMounted(async () => {
  if (user.value) {
    ...
    router.push('/login?redirect=/profile-public')
  }
})
...
const fetchProfile = async () => {
  try {
    const response = await fetch(`/api/profile-public/${route.params.uuid}`)
    ...
    if (data) {
      ...
      userData.value.bio = data.bio_profile || ''
